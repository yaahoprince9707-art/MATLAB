...
Kg=zeros(N);
F=[0,0,0,50]';
NCM=[1,1,2,200;2,2,3,50;3,3,4,100];
for i=1:3
    r=NCM(i,2:3);
    c=NCM(i,2:3);
...
x2 = linspace(L/3, 2*L/3, n_coils * points_per_coil);
...
xd2 = linspace(L/3+Um(2), 2*L/3+Um(3), n_coils * points_per_coil);
...
plot(xd1, yd1-1, 'b-', 'LineWidth', 2);
plot(xd2, yd2-1, 'r:', 'LineWidth', 2);
plot(xd3, yd3-1, 'g-.', 'LineWidth', 2);
axis equal;
xlabel('Length');
ylabel('Displacement');
title('Spring Representation');
ligend('spring1','spring2','spring3')
